...
class Node:
    def __init__(self, x: int = 0, y: int = 0, cost: float = 0.0, theta: int = 0):
        self.x = x
        self.y = y
        self.cost = cost
        self.theta = theta
...
def add_coordinates(node1: Node, node2: Node):
    ...
    new_node.theta = (node1.theta + node2.theta) % 360
            ...
            for j in range(len(dy)):
                for k in range(len(dtheta)):
                    if dx[i] == 0 and dy[j] == 0:
                        continue
                    dist = math.sqrt(dx[i] ** 2 + dy[j] ** 2)
                    # cost = cost_map[(dist, dtheta[k])]
                    cost = 1
                    self.motions.append(Node(dx[i], dy[j], cost, dtheta[k]))
