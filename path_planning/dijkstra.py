...
class Dijkstra:
	...
	class Node:
		def __init__(self, x, y, angle, cost, parent_index):
	...
	def calc_index(self, node):
		return ((node.y - self.min_y) * self.x_width + (node.x - self.min_x), node.angle)
...
def main():
	...
	rx, ry, rt = dijkstra.planning(sx, sy, st, gx, gy, gt)
