...
class BestFirstSearchPlanner:
	...
	class Node:
		def __init__(self, x, y, angle, cost, parent_index, parent):
			...
			self.angle = angle
			self.cost = cost
			self.pind = parent_index
			self.parent = parent

		def __str__(self):
			return str(self.x) + "," + str(self.y) + "," + str(
				self.angle) + ',' + str(self.cost) + "," + str(self.pind)
		...
		while True:
			...
			for i, _ in enumerate(self.motion):
				node = self.Node(current.x + self.motion[i][0],
								 ...
								 current.cost + self.motion[i][3], c_id, current)
...
def main():
	...
	rx, ry, rt = greedybestfirst.planning(sx, sy, st, gx, gy, gt)
