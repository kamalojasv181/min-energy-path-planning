...
class AStarPlanner:
	...
	def __init__(self, ox, oy, resolution, angle_resolution, rr):
		"""
	...
	class Node:
		def __init__(self, x, y, theta, cost, parent_index):
		...
		def __str__(self):
			return str(self.x) + "," + str(self.y) + "," + str(
				self.theta) + ',' + str(self.cost) + "," + str(self.parent_index)
	...
	def planning(self, sx, sy, st, gx, gy, gt):
		...
		while 1:
			...
			for i, _ in enumerate(self.motion):
				node = self.Node(current.x + self.motion[i][0],
								 ...
								 (current.theta + self.motion[i][2])%360,
		...
		rx, ry, rt = self.calc_final_path(goal_node, closed_set)
	...
	def calc_grid_index(self, node):
		return ((node.y - self.min_y) * self.x_width + (node.x - self.min_x), node.theta)
...
def main():
	...
	a_star = AStarPlanner(ox, oy, grid_size, 360, robot_radius)
	rx, ry, rt = a_star.planning(sx, sy, st, gx, gy, gt)
